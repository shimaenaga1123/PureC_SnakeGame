...
C언어로 작성되고 완전히 한국어화된 현대적인 크로스 플랫폼 뱀 게임입니다. Windows, macOS, Linux, WebAssembly를 지원하며, AI와의 대전과 다양한 게임 설정을 제공합니다.
...
### ⚙️ 게임 설정
- **게임 속도**: 메인 메뉴에서 느림/보통/빠름 선택 (좌우 화살표로 조정)
- **AI 특성**: AI 난이도 선택 화면에서 5가지 특성 선택 (좌우 화살표로 조정)
  - 균형잡힌 (기본)
  - 공격적 (상대방에게 적극적으로 접근)
  - 방어적 (안전한 플레이 위주)
  - 신중한 (위험을 매우 회피)
  - 무모한 (고위험 고수익 플레이)
- **세션 기반**: 설정은 게임 실행 중에만 유지되며 재시작시 기본값으로 복원
...
- **설정 조정**: 메뉴에서 `←→` (좌우 화살표)로 값 변경
- **게임 중**: `SPACE`로 일시정지, `ESC`로 메인 메뉴 복귀
...
### 특징
- **간단한 UI**: 설정 메뉴 없이 직관적인 인터페이스
- **실시간 조정**: 메뉴에서 좌우 화살표로 즉시 설정 변경
- **다양한 AI**: 5가지 AI 특성으로 다채로운 게임플레이
- **세션 기반 설정**: 파일 저장 없이 실행 중에만 설정 유지
- **크로스 플랫폼**: 모든 주요 플랫폼에서 동일한 경험
